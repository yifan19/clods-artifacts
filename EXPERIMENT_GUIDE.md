...
— the server URL is optional and gets auto-derived from `homeserver.yaml`'s own listener, so no
need to pass `http://localhost:8008` explicitly:
```bash
/opt/synapse-clods/venv/bin/register_new_matrix_user -c /opt/synapse-clods/homeserver.yaml
# repeat for a second account
```
This uses `registration_shared_secret` under the hood (auto-generated by `setup_https_homeserver.sh`
via Synapse's own `--generate-config`, not the broken `DATADIR/reg_key` path from the original
