# Element 5038 — Notifications updated faster than database
...
| | |
|---|---|
| Element version | `1.6.8` |
| Exact commit | `b4078012e4` |
| `element-android` branch | `5038` (also in `element-android2`) |
| Package | `im.vector.app.debug` |
| Symptom probe | `im.vector.app.features.notifications.NotifiableEventProcessor.process` |
| Rounds | 1 2 3 4 |
...
- `apk/vector-gplay-arm64-v8a-debug.apk` — the actual pre-built, working APK this bug was captured
  against (symlinked from `bm_instrument/element-bugs/element5038/results/`).
...
Results-table issue number is **5132** (the upstream PR that got reverted to cause this bug) — the bug/branch/folder are all '5038'. Don't go looking for 'element5132'.
