...
# deploys the vendored hadoop-3.0.0-SNAPSHOT, formats/starts it, runs the
# baseline HiBench terasort+pagerank workload, then for each round in [2 3 4] injects that round's instrumentation
...
MISSING=0
[ -f ../binaries/hadoop-3.0.0-SNAPSHOT.tar.gz ] || { echo "[MISSING] binaries/hadoop-3.0.0-SNAPSHOT.tar.gz (needed: hadoop_name=hadoop-3.0.0-SNAPSHOT)" >&2; MISSING=1; }
[ -f ../binaries/HiBench2.tar.gz ] || { echo "[MISSING] binaries/HiBench2.tar.gz (needed: hibench_name=HiBench2)" >&2; MISSING=1; }
if [ "$MISSING" = "1" ]; then
...
echo "== running baseline + rounds [2 3 4] =="
