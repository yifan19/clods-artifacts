# Element 7643 — Notifications are automatically dismissed
...
| Exact commit | `adaf68eb7c` |
| `element-android` branch | `7643` (also in `element-android2`) |
...
- `apk/vector-gplay-arm64-v8a-debug.apk` — the actual pre-built, working APK this bug was captured
  against (symlinked from `bm_instrument/element-bugs/element7643/results/`).
...
Round numbers are non-consecutive (1,2,3,5,7,9 — no 4/6/8) — this is copied verbatim from the checked-in round* folders, not an error.
