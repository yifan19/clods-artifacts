...
Path: `android-common/https_setup/*.sh` — plain venv + systemd on the box, no Docker needed for
the server side.

```bash
# from your workstation, inside this repo
rsync -a ../android/synapse/ <box>:~/artifacts/android/synapse/
scp android-common/https_setup/setup_https_homeserver.sh android-common/https_setup/start_synapse.sh <box>:~/
```
```bash
# on the box, as root
sudo -E ./setup_https_homeserver.sh <public-ip>   # postgres + venv install + homeserver.yaml + nginx TLS on :8448
sudo ./start_synapse.sh                            # installs + starts the synapse-clods systemd service
```
`setup_https_homeserver.sh` does an editable install (`pip install -e`) from `$SYNAPSE_SRC`
(defaults to `/home/ubuntu/artifacts/android/synapse`, override if your checkout lives elsewhere)
on whatever Python 3 the box ships. Element-516's round 2 needs a separate
[Python-Instrumentation](https://github.com/yifan19/Python-Instrumentation) setup step first — see
Part C, its bytecode rewriting requires Python ≤3.10. It ends with a
`curl http://127.0.0.1:8008/health` smoke test.
...
```bash
/opt/synapse-clods/venv/bin/register_new_matrix_user http://localhost:8008 -c /opt/synapse-clods/homeserver.yaml
# repeat for a second account
```
This uses `registration_shared_secret` under the hood (auto-generated by Synapse's own
`--generate-config` during `setup_https_homeserver.sh`, not the broken `DATADIR/reg_key` path from
the original pasted config — see the note at the top of that script) — it works regardless of
whether `enable_registration` is set, since it's an admin-side bypass, not the public signup flow.

Logs: `journalctl -u synapse-clods -f`.
...
driver instead of the plain service for this one — one-time setup, then run:
```bash
