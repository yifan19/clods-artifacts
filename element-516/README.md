...
| Exact commit | `2c2cd47993` |
| `element-android` branch | `516` (also in `element-android2`) |
| Package | `im.vector.app.debug` |
| Symptom probe | `org.matrix.android.sdk.internal.session.sync.handler.room.RoomSyncHandler.deleteLocalEchosIfNeeded` |
| Rounds | 1 2 |
...
- `apk/vector-gplay-arm64-v8a-debug.apk` — the actual pre-built, working APK this bug was captured
  against (symlinked from `bm_instrument/element-bugs/element516/results/`).
