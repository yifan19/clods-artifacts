...
# deploys the vendored hadoop-1.0.0 + hbase-0.94.27 + zookeeper-3.4.5, formats/starts it, runs the
# baseline YCSB-vs-HBase workload, then for each round in [1] injects that round's instrumentation
...
MISSING=0
[ -f ../binaries/hadoop-1.0.0.tar.gz ] || { echo "[MISSING] binaries/hadoop-1.0.0.tar.gz (needed: hadoop_name=hadoop-1.0.0)" >&2; MISSING=1; }
[ -f ../binaries/hbase-0.94.27.tar.gz ] || { echo "[MISSING] binaries/hbase-0.94.27.tar.gz (needed: hbase_name=hbase-0.94.27)" >&2; MISSING=1; }
...
echo "== running baseline + rounds [1] =="
